...
from ecod.data.proph1mpx.toolbox import dat_events_tools as dat
from ecod.data.proph1mpx.toolbox import npy_events_tools as npy_format
...
class PSEELoader(object):
    ...
    def seek_event(self, ev_count):
        ...
        if ev_count <= 0:
            ...
            self.current_event = self._ev_count - 1
        else:
            # we put the cursor at the *(ev_count - 1)*nth event
            self._file.seek(self._start + (ev_count - 1) * self._ev_size)
            # we read the timestamp of the following event (this changes the position in the file)
            self.current_time = np.fromfile(self._file, dtype=self._dtype, count=1)["t"][0]
            self.current_event = ev_count
            # this is why we go back at the right position here; not needed, because go to count - 1 above
            # self._file.seek(self._start + (ev_count) * self._ev_size)
        self.done = self._file.tell() >= self._end

    def seek_time(self, final_time, term_criterion=int(1e6)):
        """
        ...
        if final_time > self.total_time():
            ...
            self.current_event = self._ev_count
            return
        ...
        self.current_event = low + final_index
        self.current_time = final_time
        self.done = self._file.tell() >= self._end

    def seek_past_time(self, time):
        if self.current_time < time:
            raise RuntimeError(f"Seek past time was called with a time in the future: {self.current_time} vs {time}")
        if time < 0:
            raise ValueError("Time has to be bigger than 0")
        batch = int(1e6)
        while self.current_time >= time:
            self.seek_event(self.current_event - batch)
            # in case first event is already > 0 but we are searching for t in range [0, t_first_event]
            if self.current_event == 0:
                break
        buffer = np.fromfile(self._file, dtype=self._dtype, count=batch)["t"]
        index = np.searchsorted(buffer, time)
        self.seek_event(self.current_event + index + 1)

    def seek_future_time(self, time):
        if self.current_time > time:
            raise RuntimeError(f"Seek future time was called with a time in the past: {self.current_time} vs {time}")
        if time < 0:
            raise ValueError("Time has to be bigger than 0")
        # 1e6 elements are aproximately 1e6*(64+8+8+1)/8/1024/1024 = 10 MByte
        # np.searchsorted on 1e6 elements takes only ~ 5 mus!
        batch = int(1e6)
        while self.current_time < time and not self.done:
            self.seek_event(self.current_event + batch)
        self.seek_event(self.current_event - batch)
        buffer = np.fromfile(self._file, dtype=self._dtype, count=batch)["t"]
        index = np.searchsorted(buffer, time)
        # + 1 because searchsorted returns index **before** the time we search; Now, are further in the future if
